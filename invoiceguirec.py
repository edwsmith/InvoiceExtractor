...
from PyQt5.QtWidgets import (
    ...
    QLabel, QFileDialog, QTableWidget, QTableWidgetItem, QWidget, QMessageBox
)

class InvoiceExtractorApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Invoice Totals Extractor")
        self.setGeometry(300, 200, 600, 400)
        
        # Layout and Widgets
        self.layout = QVBoxLayout()
        self.label = QLabel("Select a folder containing invoice PDFs (program will search through all subfolders as well):")
    ...
    def extract_totals(self):
        ...
        self.totals = self._extract_invoice_totals(self.folder_path)
        
        if self.totals:
            self._populate_table()
            QMessageBox.information(self, "Success", f"Totals extracted successfully! Total Sum: {self.totalsum}")
    ...
    def _populate_table(self):
        ...
        for row, (filename, total) in enumerate(self.totals.items()):
            ...
            self.results_table.setItem(row, 1, QTableWidgetItem(f"£{total:.2f}"))
    
    def _extract_invoice_totals(self, folder_path):
        totals = {}
        self.totalsum = 0
        
        # Walk through all files and subdirectories in the folder recursively
                    ...
                    try:
                        with pdfplumber.open(file_path) as pdf:
                            for page in pdf.pages:
                                ...
                                if text:
                                    # Look specifically for the "Total:" label followed by an amount
                                    match = re.search(r'Total:\s*£?([\d,]+\.\d{2})', text)
                                    if match:
                                        total = match.group(1).replace(",", "")
                                        totals[file_path] = float(total)  # Use the full path as key
                                        self.totalsum = self.totalsum + float(total)
